...
# 대신 asset_version()으로 파일이 바뀌면 주소(?v=...)가 자동으로 바뀌므로,
...
def inject_asset_version():
    """css/js 주소에 붙일 버전 문자열. 파일 수정 시각을 쓰므로 배포하면 자동으로 바뀐다."""
    ...
    if _ASSET_VERSION is None:
        ...
        for name in ("common.js", "style.css"):
            try:
                ...
                pass
        _ASSET_VERSION = str(newest or int(time.time()))
...
# ---------------------------------------------------------------------------
# ⚡ 검색이 느렸던 가장 큰 이유: 예전에는 제품 1건마다 재고·브랜드·카테고리·옵션수·
# 상위제품명을 각각 따로 조회했다. 제품 300건이면 1,500번 넘게 DB를 왕복하는데,
# 원격 DB는 한 번 왕복에 수십 ms가 걸리므로 검색 한 번에 수십 초가 걸렸다.
# 지금은 결과가 몇 건이든 "묶어서 최대 5번"만 조회한다. 반환하는 항목(키)은 예전과 같다.
...
def enrich_product_rows(rows, store_id=None):
    ...
    # ---------- 2) 브랜드 / 3) 카테고리 ----------
    brand_ids = list({it["brand_id"] for it in items if it.get("brand_id")})
    category_ids = list({it["category_id"] for it in items if it.get("category_id")})
    brand_map = {}
    category_map = {}
    try:
        if brand_ids:
            cur.execute("SELECT id, name, color FROM brands WHERE id = ANY(%s)", (brand_ids,))
            brand_map = {r["id"]: (r["name"], r["color"]) for r in cur.fetchall()}
        if category_ids:
            cur.execute("SELECT id, name, color FROM categories WHERE id = ANY(%s)", (category_ids,))
    ...
    variant_map = {}
    parent_name_map = {}
    parent_ids = list({it["parent_product_id"] for it in items if it.get("parent_product_id")})
    try:
        cur.execute(
        ...
        if parent_ids:
            cur.execute("SELECT id, name FROM products WHERE id = ANY(%s)", (parent_ids,))
...
def product_row_to_dict(db, row, store_id=None):
    """단일 제품 행용 (내부적으로 enrich_product_rows를 쓰므로 결과 형식이 동일하다)."""
    if row is None:
        return None
